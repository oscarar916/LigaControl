...
function renderSchedule() {
  ...
  content.innerHTML = `<div class="public-section-heading"><div><small>Calendario oficial</small><h2>Programación de partidos</h2></div></div><div class="public-rounds">${rounds.map(({ round, matches: items }) => { const bye = teams.find((team) => !items.some((match) => match.homeId === team.id || match.awayId === team.id)); const showOrder = roundHasOfficialOrder(round, items, nextPendingRound); return `<article class="public-round-card"><header><h3>Fecha ${round}</h3><span>${showOrder ? `${items.length} partidos` : 'Orden por definir'}</span></header>${items.map((match, index) => `<div class="public-match-row${showOrder ? '' : ' order-pending'}${played(match) ? ' played-match' : ''}">${showOrder ? `<b>${index + 1}</b>` : ''}<div class="public-match-teams"><strong>${esc(teamName(match.homeId))}</strong><span class="${played(match) ? 'match-score-badge' : ''}">${esc(scheduleMatchCenter(match))}</span><strong>${esc(teamName(match.awayId))}</strong></div><div class="public-match-info"><span>${esc(dateText(match.date))}</span><strong>${esc(publicMatchTime(match, index))}</strong><span>${esc(match.venue || 'Escenario pendiente')}</span></div></div>`).join('')}${bye ? `<footer>Descansa: <strong>${esc(bye.name)}</strong></footer>` : ''}</article>`; }).join('')}</div>`;
}
...
const renderers = { schedule: renderSchedule, results: renderResults, standings: renderStandings, scorers: renderScorers, sanctions: renderSanctions };
