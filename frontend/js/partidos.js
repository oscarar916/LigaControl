...
function displayMatchTime(match, index, compactTime) { if (isEliminationAward(match)) return 'W.O. reglamentario'; return isVolleyball() && index > 0 && !compactTime ? `Partido ${index + 1} · a continuación` : displayTime(compactTime || match.time); }
function renderSchedulePreview() { const round = rounds.find((item) => item.number === activeRound); const times = roundTimes(round); document.querySelector('#schedule-times-preview').innerHTML = times.length ? round.matches.map((match, index) => `<div><strong>${isEliminationAward(match) ? 'W.O. reglamentario · validación requerida' : times[index] || `Partido ${index + 1} · a continuación`}</strong><span>${escapeHtml(teamName(match.homeId))} vs. ${escapeHtml(teamName(match.awayId))}</span></div>`).join('') : ''; }
function openScheduleModal() { const round = rounds.find((item) => item.number === activeRound); if (round.number === nextPendingRoundNumber()) applySuggestedOrder(round); const interval = matchIntervalMinutes(); const firstScheduled = round.matches.find((match) => !isEliminationAward(match) && match.time) || round.matches.find((match) => match.time) || round.matches[0]; const sharedVenue = rounds.flatMap((item) => item.matches).find((match) => match.venue)?.venue || ''; document.querySelector('#schedule-title').textContent = `Programar Fecha ${round.number}`; document.querySelector('#schedule-description').textContent = 'El orden por puntos se aplicará automáticamente. Indica la fecha, hora inicial y lugar.'; document.querySelector('#schedule-interval').textContent = isVolleyball() ? 'Vóley: hora exacta solo para el primer partido' : `Intervalo fijo: ${interval} minutos entre partidos que sí se juegan`; scheduleForm.elements.date.value = firstScheduled?.date ? String(firstScheduled.date).slice(0, 10) : ''; scheduleForm.elements.startTime.value = firstScheduled?.time ? displayTime(firstScheduled.time) : '08:00'; scheduleForm.elements.venue.value = firstScheduled?.venue || sharedVenue; scheduleStatus.textContent = ''; renderSchedulePreview(); scheduleModal.showModal(); }
...
function renderRound() {
  ...
  renderTabs();
  const orderControls = confirmed ? '<span class="confirmed-order">✓ Orden confirmado</span>' : canManageOrder ? '' : '<span class="pending-order-label">Disponible después de cerrar la fecha anterior</span>';
...
function matchScheduleText(match) {
  if (isEliminationAward(match)) return `W.O. reglamentario · ${teamName(match.walkoverTeamId)} eliminado`;
  const parts = [];
...
scheduleForm.addEventListener('submit', async (event) => { event.preventDefault(); const button = scheduleForm.querySelector('button[type="submit"]'); const round = rounds.find((item) => item.number === activeRound); const values = Object.fromEntries(new FormData(scheduleForm)); const times = roundTimes(round); button.disabled = true; scheduleStatus.textContent = 'Guardando y confirmando orden…'; try { if (fixtureSaved) { const updated = await Promise.all(round.matches.map((match, index) => apiPut('/api/matches', { id: match.id, date: values.date, time: times[index], venue: values.venue, order: index + 1, orderConfirmed: true }))); round.matches = updated.map((response) => response.data).sort((a, b) => a.order - b.order); } else { round.matches = round.matches.map((match, index) => ({ ...match, date: values.date, time: times[index], venue: values.venue, order: index + 1, orderConfirmed: true })); } editingOrderRound = 0; scheduleModal.close(); renderRound(); } catch (error) { scheduleStatus.className = 'form-status error-message'; scheduleStatus.textContent = error.message; } finally { button.disabled = false; } });
