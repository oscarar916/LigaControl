...
function roundTimes(round) { const start = scheduleForm.elements.startTime.value; const interval = matchIntervalMinutes(); if (!start) return []; return round.matches.map((_, index) => isVolleyball() && index > 0 ? '' : addMinutes(start, interval * index)); }
function displayMatchTime(match, index) { return isVolleyball() && index > 0 && !match.time ? `Partido ${index + 1} · a continuación` : displayTime(match.time); }
function renderSchedulePreview() { const round = rounds.find((item) => item.number === activeRound); const times = roundTimes(round); document.querySelector('#schedule-times-preview').innerHTML = times.length ? round.matches.map((match, index) => `<div><strong>${index === 0 ? `1. ${times[0]}` : `Partido ${index + 1} · a continuación`}</strong><span>${escapeHtml(teamName(match.homeId))} vs. ${escapeHtml(teamName(match.awayId))}</span></div>`).join('') : ''; }
function openScheduleModal() { const round = rounds.find((item) => item.number === activeRound); const interval = matchIntervalMinutes(); const sharedVenue = rounds.flatMap((item) => item.matches).find((match) => match.venue)?.venue || ''; document.querySelector('#schedule-title').textContent = `Programar Fecha ${round.number}`; document.querySelector('#schedule-description').textContent = isVolleyball() ? 'Indica únicamente la hora del primer partido. Los demás se jugarán a continuación en el mismo lugar.' : `Indica la hora del primer partido. Los siguientes comenzarán cada ${interval} minutos en el mismo lugar.`; document.querySelector('#schedule-interval').textContent = isVolleyball() ? 'Vóley: hora exacta solo para el primer partido' : `Intervalo fijo: ${interval} minutos`; scheduleForm.elements.date.value = round.matches[0]?.date ? String(round.matches[0].date).slice(0, 10) : ''; scheduleForm.elements.startTime.value = round.matches[0]?.time ? displayTime(round.matches[0].time) : '08:00'; scheduleForm.elements.venue.value = round.matches[0]?.venue || sharedVenue; scheduleStatus.textContent = ''; renderSchedulePreview(); scheduleModal.showModal(); }
...
function renderRound() {
  ...
  content.innerHTML = `<div class="round-heading"><div><h3>Fecha ${round.number}</h3><div class="print-round-details"><strong>${escapeHtml(firstMatch?.date ? displayDate(firstMatch.date) : 'Fecha pendiente')}</strong><span>${escapeHtml(firstMatch?.venue || 'Escenario pendiente')}</span></div></div><div class="button-group">${canManageOrder || showOrder ? '<button class="secondary-button" type="button" data-schedule-round>Programar fecha</button>' : ''}${orderControls}</div></div>${roundDateVenueSummary(round)}<div class="fixture-match-list compact-fixture-list">${round.matches.map((match, index) => `<article class="fixture-match${showOrder ? '' : ' order-pending'}${matchPlayed(match) ? ' played-match' : ''}">${showOrder ? `<div class="match-left-meta"><span class="match-order">${index + 1}.°</span><span class="match-time">${escapeHtml(displayMatchTime(match, index))}</span></div>` : ''}<div class="match-teams"><strong>${escapeHtml(teamName(match.homeId))}</strong><span class="versus-badge${matchPlayed(match) ? ' score-badge' : ''}">${escapeHtml(matchScoreText(match))}</span><strong>${escapeHtml(teamName(match.awayId))}</strong></div><div class="match-metadata">${matchPlayed(match) ? '<span class="finished-badge">Resultado cerrado</span>' : `<span class="points-badge subtle-points" title="Suma de puntos, solo referencia para ordenar">${(points[match.homeId] || 0) + (points[match.awayId] || 0)} pts.</span>`}</div>${canManageOrder && !confirmed ? `<div class="order-actions"><button type="button" data-move="up" data-index="${index}" ${index === 0 ? 'disabled' : ''}>Subir</button><button type="button" data-move="down" data-index="${index}" ${index === round.matches.length - 1 ? 'disabled' : ''}>Bajar</button></div>` : ''}</article>`).join('')}</div>${round.byeTeamId ? `<div class="bye-card">Descansa: <strong>${escapeHtml(teamName(round.byeTeamId))}</strong></div>` : ''}`;
