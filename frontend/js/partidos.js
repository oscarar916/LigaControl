...
function roundTimes(round) { const start = scheduleForm.elements.startTime.value; const interval = matchIntervalMinutes(); return start ? round.matches.map((_, index) => addMinutes(start, interval * index)) : []; }
...
function openScheduleModal() { const round = rounds.find((item) => item.number === activeRound); const interval = matchIntervalMinutes(); const sharedVenue = rounds.flatMap((item) => item.matches).find((match) => match.venue)?.venue || ''; document.querySelector('#schedule-title').textContent = `Programar Fecha ${round.number}`; document.querySelector('#schedule-description').textContent = `Indica la hora del primer partido. Los siguientes comenzarán cada ${interval} minutos en el mismo lugar.`; document.querySelector('#schedule-interval').textContent = `Intervalo fijo: ${interval} minutos`; scheduleForm.elements.date.value = round.matches[0]?.date ? String(round.matches[0].date).slice(0, 10) : ''; scheduleForm.elements.startTime.value = round.matches[0]?.time ? displayTime(round.matches[0].time) : '08:00'; scheduleForm.elements.venue.value = round.matches[0]?.venue || sharedVenue; scheduleStatus.textContent = ''; renderSchedulePreview(); scheduleModal.showModal(); }
