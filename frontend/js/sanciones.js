...
function paymentSummary(sanction) {
  ...
  return { ids: items.map((payment) => payment.id), status: items.every((payment) => payment.status === 'PAID') ? 'PAID' : 'PENDING', amount: items.reduce((sum, payment) => sum + Number(payment.amount || 0), 0), date: items.map((payment) => payment.date).filter(Boolean).sort().at(-1) || '' };
}
...
function filteredSanctions() {
  ...
  return sanctions.filter((sanction) => {
    const payment = paymentSummary(sanction);
    const matchesType = !type || sanction.type === type || (type === 'EXPULSION' && sanction.type === 'DOUBLE_YELLOW');
    return (!round || String(roundForSanction(sanction)) === round) && (!teamId || sanction.teamId === teamId) && matchesType && (!paymentStatus || payment?.status === paymentStatus);
  });
}
...
function sanctionLabel(type) { return type === 'YELLOW_CARD' ? '🟨 Amarilla' : type === 'DOUBLE_YELLOW' ? '🟨🟥 Doble amarilla' : type === 'RED_CARD' ? '🟥 Roja directa' : '⛔ Expulsión'; }
...
function renderGroups() {
  ...
  groups.innerHTML = teamGroups.length ? teamGroups.map(({ team, items }) => {
    const pendingTotal = items.reduce((sum, item) => { const payment = paymentSummary(item); return sum + (payment?.status === 'PENDING' ? Number(payment.amount || 0) : 0); }, 0);
    return `<article class="sanction-management-card"><header><div><small>Equipo</small><h3>${esc(team.name)}</h3></div><div><span>${items.length} sanción(es)</span><strong>S/ ${pendingTotal.toFixed(2)} pendiente</strong></div></header><div class="table-wrap"><table><thead><tr><th>Jugador</th><th>Fecha</th><th>Tarjeta</th><th>Suspensión</th><th>Monto</th><th>Pago</th><th></th></tr></thead><tbody>${items.map((item) => {
      const payment = paymentSummary(item); const paid = payment?.status === 'PAID'; const round = roundForSanction(item);
      return `<tr><td><strong>${esc(playerName(item.playerId))}</strong></td><td>${round ? `Fecha ${round}` : 'Sin fecha'}</td><td>${sanctionLabel(item.type)}</td><td class="suspension-cell">${suspensionText(item)}</td><td><strong>S/ ${Number(item.amount || 0).toFixed(2)}</strong></td><td><span class="badge ${paid ? 'paid-badge' : 'pending-badge'}">${paid ? 'Pagado' : 'Pendiente'}</span>${paid && payment.date ? `<small class="payment-date">${esc(payment.date)}</small>` : ''}</td><td>${payment ? `<label class="payment-state-editor"><span class="sr-only">Estado de pago de ${esc(playerName(item.playerId))}</span><select data-payment-state="${payment.ids.join(',')}" data-current-state="${payment.status}"><option value="PENDING"${paid ? '' : ' selected'}>Pendiente</option><option value="PAID"${paid ? ' selected' : ''}>Pagado</option></select></label>` : ''}</td></tr>`;
...
async function load() {
  try {
    ...
    sanctions = mergeDoubleYellowSanctions((data.sanctions || []).filter((sanction) => playerIds.has(sanction.playerId))); const sanctionIds = new Set(sanctions.flatMap((sanction) => sanction.sourceSanctionIds || [sanction.id]));
...
groups.addEventListener('change', async (event) => { const select = event.target.closest('[data-payment-state]'); if (!select) return; const previousState = select.dataset.currentState; const nextState = select.value; if (previousState === nextState) return; const label = nextState === 'PAID' ? 'pagada' : 'pendiente'; if (!confirm(`¿Confirmas que deseas marcar esta multa como ${label}?`)) { select.value = previousState; return; } select.disabled = true; try { for (const paymentId of select.dataset.paymentState.split(',').filter(Boolean)) await savePaymentState(paymentId, nextState === 'PAID'); renderSummary(); renderGroups(); status.textContent = `Multa marcada como ${label}.`; status.className = 'form-status success-message'; } catch (error) { select.value = previousState; select.disabled = false; status.textContent = error.message; status.className = 'form-status error-message'; } });
