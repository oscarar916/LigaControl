...
groups.addEventListener('click', async (event) => { const button = event.target.closest('[data-mark-paid]'); if (!button) return; if (!confirm('¿Confirmas que esta multa fue pagada?')) return; button.disabled = true; try { const response = await apiPut('/api/payments', { id: button.dataset.markPaid, paid: true }); payments = payments.map((item) => item.id === response.data.id ? response.data : item); renderSummary(); renderGroups(); status.textContent = 'Pago registrado correctamente.'; status.className = 'form-status success-message'; } catch (error) { status.textContent = error.message; status.className = 'form-status error-message'; button.disabled = false; } });
